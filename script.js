...
const CROP_DATA = {
    ...
    eventFlytrap: { name: 'Venus Flytrap', icon: '🪴', buyPrice: 0, value: 50, growTime: 20, rarity: 'common', unlock: 999, isEvent: true },
    eventBloodRose: { name: 'Blood Rose', icon: '🌹', buyPrice: 0, value: 150, growTime: 40, rarity: 'uncommon', unlock: 999, isEvent: true },
    eventAlienPod: { name: 'Alien Pod', icon: '👽', buyPrice: 0, value: 400, growTime: 60, rarity: 'rare', unlock: 999, isEvent: true },
    eventEyeTree: { name: 'Eye Tree', icon: '👁️', buyPrice: 0, value: 1000, growTime: 90, rarity: 'epic', unlock: 999, isEvent: true },
    eventYggdrasil: { name: 'Yggdrasil', icon: '🌳', buyPrice: 0, value: 10000, growTime: 200, rarity: 'mythic', unlock: 999, isEvent: true }
};
...
let gameState = {
    ...
    weatherTimer: 0,
    totalHarvested: 0,
    hungryPlant: { active: false, seed: null, cooldown: 120 },
    lastSave: Date.now()
};
    ...
    try {
        ...
        if (saved) {
            ...
            if (data && typeof data === 'object') {
                ...
                gameState.hungryPlant = gameState.hungryPlant || { active: false, seed: null, cooldown: 120 };
...
function updateHungryPlant() {
    if (!gameState.hungryPlant) {
        gameState.hungryPlant = { seed: null };
    }
    
    if (!gameState.hungryPlant.seed) {
        const playerLevel = gameState.level || 1;
        const unlocked = Object.keys(CROP_DATA).filter(k => CROP_DATA[k].unlock <= playerLevel && !CROP_DATA[k].isEvent);
        if (unlocked.length > 0) {
            const randomSeed = unlocked[Math.floor(Math.random() * unlocked.length)];
            gameState.hungryPlant.seed = randomSeed;
            renderHungryPlant();
        }
    }
}
...
function renderHungryPlant() {
    ...
    if (gameState.hungryPlant && gameState.hungryPlant.seed) {
        const seedKey = gameState.hungryPlant.seed;
...
function feedHungryPlant() {
    if (!gameState.hungryPlant || !gameState.hungryPlant.seed) return;
    const seedKey = gameState.hungryPlant.seed;
...
function skipHungryPlant() {
    ...
    if ((gameState.money || 0) >= 10) {
        gameState.money -= 10;
        gameState.hungryPlant.seed = null;
        showFloatingText(window.innerWidth / 2, window.innerHeight / 2, '-💰10', 'money');
        updateHungryPlant();
        renderAll();
    } else {
...
function openSeedPack() {
    const rewards = [];
    for (let i = 0; i < 5; i++) {
        rewards.push(getRandomEventSeed());
    }
...
function getRandomEventSeed() {
    const rand = Math.random();
    if (rand < 0.005) return 'eventYggdrasil'; // 0.5%
    if (rand < 0.045) return 'eventEyeTree';   // 4%
    if (rand < 0.170) return 'eventAlienPod';  // 12.5%
    if (rand < 0.450) return 'eventBloodRose'; // 28%
    return 'eventFlytrap';                     // 55%
}

function showSeedPackModal(counts) {
